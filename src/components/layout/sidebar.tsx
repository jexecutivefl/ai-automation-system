...
const navItems = [
  ...
  { href: '/logs', label: 'Activity Log', icon: LogsIcon },
    ...
    <aside className="fixed left-0 top-0 h-screen w-60 bg-slate-900 text-slate-300 flex flex-col z-50">
      <div className="px-5 py-5 border-b border-slate-700/50">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h1 className="text-sm font-semibold text-white leading-tight">AI Automation</h1>
            <p className="text-[10px] text-slate-500 leading-tight">Workflow System</p>
          </div>
        </div>
      </div>
      ...
      <nav className="flex-1 py-3 px-3 space-y-0.5 overflow-y-auto">
        {navItems.map(({ href, label, icon: Icon }) => {
          ...
          return (
            ...
            >
              <Icon className={`w-[18px] h-[18px] ${isActive ? 'text-blue-400' : 'text-slate-500'}`} />
              {label}
            </Link>
          )
        })}
